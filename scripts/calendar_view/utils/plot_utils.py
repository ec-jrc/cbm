...
def get_year_months_dict_for_weekly(tiles_to_download):
    ...
    for tile_name in tiles_to_download:
        # megyunk vegig a datumokon es csinalunk beloluk year_month-ot
        acq_date = download_utils.get_acquisition_date_from_tile_name(tile_name)
        year_month = int(acq_date.split('-')[0] + acq_date.split('-')[1])
 
        # ha ehhez a year_month-hoz meg nincs kulcs, akkor hozzaadjuk uresen
        if not year_month in year_month_dict:
            year_month_dict[year_month] = {}
        day = int(acq_date.split('-')[2])
        
        # aztan megnezzuk, hogy az aktualis datum melyik hetbe, vagy felhetbe esik es ha nincs olyan
        # kulcs az al-dictionary-ban, akkor hozzaadjuk
        if day >= 1 and day <= 7:
...
def calendar_view_half_weekly(parcel_id, crop, out_tif_folder_base, tiles_to_download, parcel, vector_file_name, parcel_id_column, logfile):   
    ###############################################################
    ...
    fig_size_x = 16
    fig_size_y = fig_size_x * (num_rows / num_cols) * get_parcel_extent_ratio(parcel, buffer_size_meter)
    ...
    #     print(acq_date, act_row, act_col)
    ...
    pyplot.tight_layout()        
    output_jpg_file = output_jpg_folder + "/" + chip_folder + "_batch.jpg"
    fig.savefig(output_jpg_file, dpi=100)  
    pyplot.close(fig)
    print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "\t", parcel_id,  "\tplot_utils.calendar_view_half_weekly:\t", "{0:.3f}".format(time.time() - start), file=fout)
    fout.close()
...
def calendar_view_half_weekly_more_param(parcel_id, crop, out_tif_folder_base, tiles_to_download, parcel, vector_file_name, 
    ...
    year_months_dict = get_year_months_dict(acq_dates)
    year_months_dict_half_weekly = get_year_months_dict_for_half_weekly(tiles_to_download)
    # print(year_months_dict_half_weekly)
    first_year_month = min(year_months_dict.keys())
    number_of_year_months = get_number_of_rows(acq_dates)
    num_rows = number_of_year_months + 1
    num_cols = 9
...
def calendar_view_half_weekly_more_param_with_jrc_logo(parcel_id, crop, out_tif_folder_base, tiles_to_download, parcel, vector_file_name, 
                                            parcel_id_column, buffer_size_meter, vector_color, logfile, jrc_logo):   
    ###############################################################
    # calendar_view_visualization half weekly display
    ############################################################### 
    fout = open(logfile, 'a')
    ...
    logo_image = rasterio.open(jrc_logo)    
    logo_to_plot = logo_image.read()   
    rasterio.plot.show(logo_to_plot, ax=a[0][0]) 
    ...
    pyplot.tight_layout()        
    output_jpg_file = output_jpg_folder + "/" + chip_folder + "_buff" + str(buffer_size_meter) + "m_" + vector_color + ".tif"
    fig.savefig(output_jpg_file, dpi=100)  
    pyplot.close(fig)
    print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "\t", parcel_id,  "\tplot_utils.calendar_view_half_weekly_more_param:\t", "{0:.3f}".format(time.time() - start), file=fout)
    fout.close()
   ...
   # # we remove the axes showing the coordinates which is anyway meaningless
    ...
    #     print(acq_date, act_row, act_col)
        ...
        if act_col > 0:
            ...
            #             plt.title(f"Histogram {sensor} of {date} for parcel {pid} ({cropname})")
                # pyplot.xlabel("NDVI", fontsize=16)
                # pyplot.ylabel("Pixels per bin", fontsize=16)
    ...
    pyplot.tight_layout()        
    # output_hist_file = output_hist_folder + "/" + chip_folder  + ".jpg"
    fig.savefig(output_hist_file, dpi=100)  
    pyplot.close(fig)
...
def plot_scatter_for_one_parcel(parcel_id, crop, out_tif_folder_base, tiles_to_download, parcel, vector_file_name, 
                                            parcel_id_column, logfile):   
    ...
    chip_folder = str(parcel_id) + '_' + crop
    output_scatter_folder = out_tif_folder_base + "/overview_scatter_half_weekly"
   ...
   # # we remove the axes showing the coordinates which is anyway meaningless
    ...
    for merged_tif_file in merged_tif_files:
    ...
    #     print(acq_date, act_row, act_col)
        ...
        if act_col > 0:
            with rasterio.open(merged_tif_file) as src_image:
                ...
                red = src_image_masked[0,:]
                nir = src_image_masked[2,:]
                a[act_row][act_col].scatter(nir,red, marker = "x")
                
                
                xmin, xmax, ymin, ymax = a[act_row][act_col].axis()
 
                if len(np.nonzero(red)[0]) > 0:
                    ymin = np.min(red[np.nonzero(red)])
                if len(np.nonzero(nir)[0]) > 0:
                    xmin = np.min(nir[np.nonzero(nir)])
    ...
    pyplot.tight_layout()        
    # output_scatter_file = output_scatter_folder + "/" + chip_folder  + ".jpg"
    fig.savefig(output_scatter_file, dpi=100)  
    pyplot.close(fig)
    print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "\t", parcel_id,  "\tplot_utils.plot_histogram_for_one_parcel:\t", "{0:.3f}".format(time.time() - start), file=fout)
    fout.close()
    print(f"Histogram view: {time.time() - start} seconds")
...
def plot_scatter_for_one_parcel_fixed_scale(parcel_id, crop, out_tif_folder_base, tiles_to_download, parcel, vector_file_name, 
                                            parcel_id_column, logfile):   
    ...
    chip_folder = str(parcel_id) + '_' + crop
    output_scatter_folder = out_tif_folder_base + "/overview_scatter_half_weekly_fixed_scale"
    ...
    #     print(acq_date, act_row, act_col)
        ...
        if act_col > 0:
            with rasterio.open(merged_tif_file) as src_image:
                ...
                red = src_image_masked[0,:]
                nir = src_image_masked[2,:]
                a[act_row][act_col].scatter(nir,red, marker = "x", s = 0.5)
...
def plot_scatter_for_one_parcel_fixed_scale_cumulative_red_swir(parcel_id, crop, out_tif_folder_base, tiles_to_download, parcel, vector_file_name, 
                                            parcel_id_column, logfile):   
    ...
    chip_folder = str(parcel_id) + '_' + crop
    output_scatter_folder = out_tif_folder_base + "/overview_scatter_half_weekly_fixed_scale_cumulative_red_swir"
    ...
    #     print(acq_date, act_row, act_col)
        ...
        if act_col > 0:
            with rasterio.open(merged_tif_file) as src_image:
                ...
                red.append(src_image_masked[2,:])
                swir.append(src_image_masked[1,:])
                
                if date_index == 0:
                    # for the first date we display only that date with red
                    a[act_row][act_col].scatter(red[date_index],swir[date_index], marker = "x", s = 1, c="red")
                else:
                    # for the other dates we display all the old dates with blue and the current date with red
                    for old_date_index in range (0,date_index):
                        a[act_row][act_col].scatter(red[old_date_index],swir[old_date_index], marker = "x", s=1,c="blue")
                    a[act_row][act_col].scatter(red[date_index],swir[date_index], marker = "x", s=1,c="red")
...
def calendar_view_s1_bs_imagettes(parcel_id, crop, out_s1_bs_folder_rescale_lut, parcel, vector_file_name, 
    ...
    # print(acq_dates)
    # print(acquisition_dates_and_s1_bs_lut_files_dict)
    chip_folder = str(parcel_id) + '_' + crop
...
def calendar_view_half_weekly_more_param_cir(parcel_id, crop, out_tif_folder_base, tiles_to_download, parcel, vector_file_name, 
                                            parcel_id_column, buffer_size_meter, vector_color, logfile, jpg_resolution, acq_dates, merged_lut_files):   
    ###############################################################
    # calendar_view_visualization half weekly display
    ############################################################### 
    ...
    chip_folder = str(parcel_id) + '_' + crop
    output_jpg_folder = out_tif_folder_base + "/overview_jpg_half_weekly_cir"
    # buffer_size_meter = 50
    pixel_size_meter = 10
    # vector_color = 'black'
    ...
    parcel_extent_ratio = get_parcel_extent_ratio(parcel, buffer_size_meter)
    fig_size_x = 16
    fig_size_y = fig_size_x * (num_rows / num_cols) * parcel_extent_ratio
    # fig_size_y = fig_size_y - fig_size_x*2
    
    # print("fig_size_x= ", fig_size_x) 
    # print("fig_size_y= ", fig_size_y) 
    # print("parcel extent ratio= ", parcel_extent_ratio) 
    ...
    #     print(acq_date, act_row, act_col)
        ...
        if act_col > 0:
            ...
            win = get_window(parcel, src_image, buffer_size_meter, pixel_size_meter)
            #print(win)
            toPlot = src_image.read((1,3,3),window=win)              
            a[act_row][act_col].set_title(acq_date, verticalalignment='center', fontdict={'fontsize': 8, 'fontweight': 'medium'})
            # print(merged_lut_file)
            rasterio.plot.show(toPlot, ax=a[act_row][act_col], transform=src_image.window_transform(win)) 
            parcel.plot(ax=a[act_row][act_col], facecolor='none', edgecolor=vector_color) 
