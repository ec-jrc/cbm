...
def cbm():
    ...
    path_data = f"{config.get_value(['paths', 'temp'])}cbm/"
    ...
    cbm_info = HTML(
        value="""Main src.ipycbm Jupyter Notebooks graphical panel to run all the required procedures <br>
        for Checks by Monitoring (CbM) with a sample dataset.<br>
        """,
        placeholder='CbM Information',
    )
