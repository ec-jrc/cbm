...
class FileReader:
    ...
    def validate_file_extension(self):
        ...
        if file_extension.lower() not in self.valid_extensions:
            raise Exception(f"Invalid file format (supported files: {', '.join(self.valid_extensions)})")

    def get_first_valid_file_from_zip(self, zip_ref):
        """Get the first valid file from a ZIP archive."""
        valid_files = [f for f in zip_ref.namelist() if any(f.lower().endswith(ext) for ext in self.valid_extensions)]
        if not valid_files:
    ...
    def read_file(self) -> dd.DataFrame:
        ...
        if file_extension.lower() == '.zip':
            with zipfile.ZipFile(self.file_fullpath, 'r') as zip_ref:
                ...
                extracted_file_path = os.path.join(os.path.dirname(self.file_fullpath), filename, first_valid_file)
                zip_ref.extract(first_valid_file, path=os.path.dirname(extracted_file_path))
    ...
    def read_csv_dask(self, datafile) -> dd.DataFrame:
        ...
        try:
            ...
            if delimiter != ';':
                raise Exception(f"Error: CSV file should use a semicolon (;) as delimiter. Detected: {delimiter}")
            header = [col.lower() for col in sample.splitlines()[0].split(delimiter)]

            if 'geometry' in header:
                header = [f'{self.table_type}_geom' if col == 'geometry' else col for col in header]

            dtype_dict = self.get_csv_dtype_dict()
            return dd.read_csv(datafile, blocksize=10e6, delimiter=delimiter,
                               assume_missing=True, dtype=dtype_dict,
                               names=header, header=0)
    ...
    def get_csv_dtype_dict(self):
        ...
        if self.table_type == 'gsa':
            dtype_dict.update({'gsa_hol_id': 'object', 'main_crop': 'object', 'catch_crop': 'object'})
        return dtype_dict

    def read_shapefile(self, datafile) -> dd.DataFrame:
        """Read a shapefile into a Dask DataFrame."""
        gdf = gpd.read_file(datafile)
        gdf[f'{self.table_type}_geom'] = gdf['geometry'].apply(lambda x: x.wkt if x is not None else None)
...
def progressbar(total, progress, label='', length=20, color='WARNING'):
    ...
    try:
        ...
        if progress >= total:
            ...
            print(f"\r{colors('OKGREEN')}|{bar}| {percent}% - {label} Process Completed. {colors('ENDC')}")
